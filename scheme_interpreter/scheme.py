...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if vals is nil:
                raise SchemeError
            a.define(formals.first, vals.first)
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        a = self.env.make_child_frame(self.formals, args)
        return a
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        ans = scheme_eval(expressions.rest.first, env)
        env.define(target, ans)
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        ans = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first, ans)
        return target.first
    else:
...
def do_macro_form(expressions, env):
    check_form(expressions, 2)
    formals = expressions.first
    check_formals(formals)
    return MacroProcedure(formals, expressions.rest, env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    while check and expressions.rest is not nil:
        ans = scheme_eval(expressions.first,env)
        check = scheme_truep(ans)
        if not check:
            return ans
        expressions = expressions.rest
...
def do_define_macro(expressions, env):
    ...
    target = expressions.first
    check_form(target,1)
    if scheme_symbolp(target.first) and isinstance(target, Pair):
        ans = do_macro_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first, ans)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
